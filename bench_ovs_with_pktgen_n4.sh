...
function run_pktgen_test() {
    ...
    sleep 1
    ip netns exec $NS_CLIENT bash -c "echo 'rem_device_all' > /proc/net/pktgen/kpktgend_6 2>/dev/null"
    sleep 1

    # Step 2: Bind device to CPU thread 0
    echo "    Binding $SRC_IF to pktgen thread..."
    if ! ip netns exec $NS_CLIENT bash -c "echo 'add_device $SRC_IF' > /proc/net/pktgen/kpktgend_6 2>&1"; then
